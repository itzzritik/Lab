	...
	return (
		<div className="mx-auto max-w-5xl p-8">
			...
			<div className="mb-10 animate-fade-up pt-2">
				<p className="mb-1 text-sm text-base-content/50">{today}</p>
				<h1 className="mb-1 font-display text-3xl font-bold tracking-tight">Welcome back</h1>
				<p className="text-base-content/60">Your creative experiments at a glance</p>
			</div>

			{/* Stats */}
			<div className="mb-10 animate-fade-up grid grid-cols-2 gap-4 sm:grid-cols-4" style={{ animationDelay: "60ms" }}>
				<div className="flex items-center gap-4 rounded-xl border border-base-content/8 bg-base-100 p-4">
					...
					<div>
						...
						<p className="text-xs text-base-content/50">Experiments</p>
					</div>
				</div>
			</div>
			...
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{experiments.map((exp, i) => (
					<Link
						key={exp.slug}
						href={`/${exp.slug}`}
						className="group animate-fade-up flex gap-4 rounded-xl border border-base-content/8 bg-base-100 p-5 transition-all dur hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-lg"
						...
						<div className="min-w-0 flex-1">
							...
							{exp.description && <p className="mt-0.5 truncate text-sm text-base-content/50">{exp.description}</p>}
						</div>
						<Icon
							code="f054"
							type="solid"
							className="ico-3 shrink-0 self-center text-base-content/20 transition-all dur group-hover:translate-x-0.5 group-hover:text-primary/50"
						/>
					</Link>
				))}

				{experiments.length === 0 && (
					<div className="animate-fade-in col-span-full py-20 text-center text-base-content/40">
