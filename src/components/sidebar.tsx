			...
			<aside
				...
				<div className="flex h-16 shrink-0 items-center px-4">
					...
					<button
						type="button"
						onClick={toggle}
						className="btn btn-ghost btn-sm btn-square text-base-content/50 hover:bg-base-content/10 hover:text-base-content md:hidden">
...
function NavItem({ href, icon, label, active, collapsed }: { href: string; icon: string; label: string; active: boolean; collapsed: boolean }) {
	const link = (
		<Link
			href={href}
			className={cn(
				"group relative flex items-center rounded-xl p-2 transition-all duration-300 ease-spring",
				active
					? "bg-primary/10 text-primary shadow-sm"
					: "text-base-content/60 hover:scale-[1.02] hover:bg-base-content/5 hover:text-base-content active:scale-[0.98]",
			)}>
			...
			<span
				className={cn(
					"overflow-hidden whitespace-nowrap font-medium text-[0.875rem] transition-all duration-500 ease-spring",
					collapsed ? "ml-0 max-w-0 opacity-0" : "ml-3 max-w-[200px] opacity-100",
				)}>
				{label}
			</span>

			{active && !collapsed && <div className="absolute top-1/2 right-2 h-4 w-1 -translate-y-1/2 rounded-full bg-primary" />}
		</Link>
	);
