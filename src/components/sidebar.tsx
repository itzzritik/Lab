...
export function Sidebar({ experiments }: { experiments: Experiment[] }) {
	...
	return (
		<aside
			className={cn(
				"flex h-full shrink-0 flex-col border-r border-base-content/10 bg-base-100 transition-all dur ease-in-out",
				collapsed ? "w-14" : "w-60",
			)}
		>
			{/* Header */}
			<div className={cn("flex h-14 items-center border-b border-base-content/10", collapsed ? "justify-center" : "px-3")}>
				...
				{!collapsed && (
					<span className="ml-2 animate-fade-in font-display text-base font-bold tracking-tight">Ritik's Lab</span>
				)}
			</div>
			...
			<nav className={cn("flex-1 space-y-0.5 py-3", collapsed ? "overflow-visible px-1.5" : "overflow-y-auto px-2.5")}>
				...
				{!collapsed && (
					<p className="mb-1.5 mt-5 px-2.5 text-[0.6rem] font-semibold uppercase tracking-[0.15em] text-base-content/40">
						Experiments
					</p>
				)}
				...
				{experiments.length === 0 && !collapsed && (
					<p className="px-2.5 py-2 text-xs text-base-content/40">No experiments yet</p>
				)}
			</nav>
